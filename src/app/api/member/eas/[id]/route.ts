...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const existing = await prisma.sharedExpertAdvisor.findFirst({
  ...
  if (file && file instanceof File && file.size > 0) {
    ...
    try {
      const stored = await storeMtMarketplaceFile(session.user.id, file, buf, "eas", "expert-advisor");
    ...
    const oldLocal = localMtMarketplaceFileAbsolutePath(existing.fileUrl);
    if (oldLocal) {
      try {
  ...
  try {
    const row = await prisma.sharedExpertAdvisor.update({
      where: { id },
      data: {
        title,
        description,
    ...
    return NextResponse.json({
      ok: true,
      ea: { ...row, priceIdr: Number(row.priceIdr) },
    });
  } catch (e) {
    console.error("member eas PATCH", e);
    return NextResponse.json({ error: "Gagal memperbarui EA" }, { status: 500 });
  }
}
