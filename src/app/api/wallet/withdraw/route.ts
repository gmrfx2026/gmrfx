...
import { Prisma, WithdrawMethod } from "@prisma/client";
import { z } from "zod";
...
export async function GET() {
  ...
  try {
    ...
    if (
      ...
      console.warn("[wallet/withdraw] Tabel/kolom penarikan belum tersedia di database; kembalikan daftar kosong.");
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (
      ...
      console.warn("[wallet/withdraw] Fitur penarikan belum tersedia penuh di database; tolak sementara.");
      return NextResponse.json(
        { error: "Fitur penarikan belum siap di server. Silakan deploy migrasi database terlebih dahulu." },
        { status: 503 }
      );
    }
    throw error;
