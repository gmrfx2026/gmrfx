...
export const metadata: Metadata = { title: "Marketplace EA — Admin GMR FX" };
...
export default async function AdminEaPage() {
  const items = await prisma.sharedExpertAdvisor.findMany({
  ...
  return (
    <div>
      <div className="mb-5">
        <h1 className="text-xl font-bold text-gray-900">Marketplace Expert Advisor</h1>
        <p className="mt-0.5 text-sm text-gray-500">Kelola semua EA yang diunggah oleh member.</p>
      </div>
            ...
            {items.map((item) => (
              <tr key={item.id} className={item.published ? "" : "bg-gray-50/60"}>
                <td className="px-4 py-3 max-w-[180px]">
                  <Link href={`/ea/${item.slug}`} target="_blank" className="font-medium text-blue-600 hover:underline line-clamp-2">
                    {item.title}
                  </Link>
                </td>
                ...
                <td className="px-4 py-3">
                  <MarketplaceActionButtons
                    ...
                    apiBase="/api/admin/marketplace/ea"
                  />
                </td>
              </tr>
            ))}
            {items.length === 0 && (
              <tr><td colSpan={9} className="px-4 py-8 text-center text-gray-400 text-sm">Belum ada EA.</td></tr>
            )}
          </tbody>
        </table>
      </div>
